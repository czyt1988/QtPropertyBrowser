...
void QtButtonPropertyBrowserPrivate::updateLater()
{
    QMetaObject::invokeMethod(
        q_ptr, [ this ] { slotUpdate(); }, Qt::QueuedConnection);
}
...
void QtButtonPropertyBrowserPrivate::propertyInserted(QtBrowserItem* index, QtBrowserItem* afterIndex)
{
    ...
    if (!parentItem) {
        ...
        if (!parentItem->container) {
            ...
            q_ptr->connect(parentItem->button, &QAbstractButton::toggled, q_ptr, [ this ](bool checked) {
                slotToggled(checked);
            });
    ...
    if (newItem->widget) {
        QObject::connect(newItem->widget, &QWidget::destroyed, q_ptr, [ this ] { slotEditorDestroyed(); });
